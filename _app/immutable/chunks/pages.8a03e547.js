...
`,1)[0],x=0;this.options.pedantic?(x=2,d=v.trimStart()):(x=t[2].search(/[^ ]/),x=x>4?1:x,d=v.slice(x),x+=t[1].length);let R=!1;if(!v&&/^ *$/.test(S)&&(p+=S+`
`,n=n.substring(S.length+1),E=!0),!E){const he=new RegExp(`^ {0,${Math.min(3,x-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),Ge=new RegExp(`^ {0,${Math.min(3,x-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),be=new RegExp(`^ {0,${Math.min(3,x-1)}}(?:\`\`\`|~~~)`),Ce=new RegExp(`^ {0,${Math.min(3,x-1)}}#`);for(;n;){const Re=n.split(`
`,1)[0];if(S=Re,this.options.pedantic&&(S=S.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),be.test(S)||Ce.test(S)||he.test(S)||Ge.test(n))break;if(S.search(/[^ ]/)>=x||!S.trim())d+=`
`+S.slice(x);else{if(R||v.search(/[^ ]/)>=4||be.test(v)||Ce.test(v)||Ge.test(v))break;d+=`
`+S}!R&&!S.trim()&&(R=!0),p+=Re+`
`,n=n.substring(Re.length+1),v=S.slice(x)}}u.loose||(y?u.loose=!0:/\n *\n *$/.test(p)&&(y=!0));let L=null,X;this.options.gfm&&(L=/^\[[ xX]\] /.exec(d),L&&(X=L[0]!=="[ ] ",d=d.replace(/^\[[ xX]\] +/,""))),u.items.push({type:"list_item",raw:p,task:!!L,checked:X,loose:!1,text:d,tokens:[]}),u.raw+=p}u.items[u.items.length-1].raw=p.trimEnd(),u.items[u.items.length-1].text=d.trimEnd(),u.raw=u.raw.trimEnd();for(let E=0;E<u.items.length;E++)if(this.lexer.state.top=!1,u.items[E].tokens=this.lexer.blockTokens(u.items[E].text,[]),!u.loose){const v=u.items[E].tokens.filter(x=>x.type==="space"),S=v.length>0&&v.some(x=>/\n.*\n/.test(x.raw));u.loose=S}if(u.loose)for(let E=0;E<u.items.length;E++)u.items[E].loose=!0;return u}}html(n){const t=this.rules.block.html.exec(n);if(t)return{type:"html",block:!0,raw:t[0],pre:t[1]==="pre"||t[1]==="script"||t[1]==="style",text:t[0]}}def(n){const t=this.rules.block.def.exec(n);if(t){const s=t[1].toLowerCase().replace(/\s+/g," "),o=t[2]?t[2].replace(/^<(.*)>$/,"$1").replace(this.rules.inline._escapes,"$1"):"",u=t[3]?t[3].substring(1,t[3].length-1).replace(this.rules.inline._escapes,"$1"):t[3];return{type:"def",tag:s,raw:t[0],href:o,title:u}}}table(n){const t=this.rules.block.table.exec(n);if(t){if(!/[:|]/.test(t[2]))return;const s={type:"table",raw:t[0],header:Sc(t[1]).map(o=>({text:o,tokens:[]})),align:t[2].replace(/^\||\| *$/g,"").split("|"),rows:t[3]&&t[3].trim()?t[3].replace(/\n[ \t]*$/,"").split(`
...
type Query {
  ...
  """
  customerOffersList(identityValue: String!, input: BenefitInput): CustomerOffersResponse
  """
  <div>
  <img src="https://cdn-icons-png.flaticon.com/512/6928/6928921.png" alt="verified" width="32" style="vertical-align:middle"/>
  <img src="https://uxwing.com/wp-content/themes/uxwing/download/checkmark-cross/done-icon.png" alt="developed" width="32" style="vertical-align:middle"/>
  <p>
  Return Points or Cashpot Rewards list (along with expiration dates).
  </p>
  </div>
  """
  customerPointsOrRewardsList(identityValue: String!, input: CustomerPointsOrRewardsInput, pagination: PaginationQuery): CustomerPointsOrRewardsResponse
  """
  <div>
  <img src="https://cdn-icons-png.flaticon.com/512/6928/6928921.png" alt="verified" width="32" style="vertical-align:middle"/>
  <img src="https://cdn.icon-icons.com/icons2/3641/PNG/512/play_green_button_icon_227849.png" alt="progress" width="32" style="vertical-align:middle"/>
...
type Mutation {
  ...
  """
  redeemRewardsForOrder(identityValue: String!, redeemRewardsForOrderInput: RedeemRewardsForOrderInput!): RedeemRewardsForOrderResponse
  """
  <div>
  <img src="https://cdn-icons-png.flaticon.com/512/6928/6928921.png" alt="verified" width="32" style="vertical-align:middle"/>
  <img src="https://usdvexperts.com/wp-content/uploads/2016/02/c286ca98c6c79cf50000cab7a5344393-e1458196160872.png" alt="pending" width="32" style="vertical-align:middle"/>
  ...
  """
  fulfillOrder(identityValue: String!, fulfillOrderInput: FulfillOrderInput!): FulfillOrderResponse
  """
  <div>
  <img src="https://cdn-icons-png.flaticon.com/512/6928/6928921.png" alt="verified" width="32" style="vertical-align:middle"/>
  <img src="https://usdvexperts.com/wp-content/uploads/2016/02/c286ca98c6c79cf50000cab7a5344393-e1458196160872.png" alt="pending" width="32" style="vertical-align:middle"/>
...
type VerifyAvailableRewardsResponse {
  ...
  pointsAndRewardsAvailable: PointsAndRewardsAvailable
}
...
type Customer {
  ...
  points: Balances
  "List of rewards with expiration dates"
  rewards: Balances
...
type Balances {
  balance: Balance
  expireationScheduleList: [ExpirationSchedule]
}
...
type BasketSummary {
  ...
  """The redemption cashpot used for this request"""
  redemptionChannel: String
}
...
`;class r_{constructor(n){De(this,"mapping");this.mapping=n}getFor(n){return this.mapping.get(n.name)}}var ar;(function(i){i.UNION="union",i.ARGUMENT="argument",i.FIELD="field"})(ar||(ar={}));function s_(i){const n=new Map;return o_(i,t=>{En(t)&&a_(t,s=>{const o=ga(s.type);aa(n,o).references.push({kind:ar.FIELD,parent:t,by:s}),u_(s,l=>{const p=ga(l.type);aa(n,p).references.push({kind:ar.ARGUMENT,field:s,type:t,by:l})})}),Kn(t)&&t.getTypes().forEach(s=>{aa(n,s).references.push({kind:ar.UNION,by:t})})}),new r_(n)}function o_(i,n){Object.entries(i.getTypeMap()).forEach(([,t])=>{t.name.startsWith("__")||n(t)})}function a_(i,n){Object.entries(i.getFields()).forEach(([,t])=>{n(t)})}function u_(i,n){Object.entries(i.args).forEach(([,t])=>{n(t)})}function aa(i,n){let t=i.get(n.name);return t||(t={references:[]},i.set(n.name,t)),t}function ga(i){return ln(i)||pi(i)?ga(i.ofType):i}const fn=v_(),ya=wa(ba.DIRECTIVES,[]);var Yc;const l_=Os((Yc=fn.getQueryType())==null?void 0:Yc.getFields());var Qc;const c_=Os((Qc=fn.getMutationType())==null?void 0:Qc.getFields());var Wc;const f_=Os((Wc=fn.getSubscriptionType())==null?void 0:Wc.getFields()),xa=Ct.keyBy(Vf(),i=>i.name.toLocaleLowerCase()),p_=Ct.mapValues(xa,i=>{const n=ya.find(t=>(t==null?void 0:t.name)===i.name||(t==null?void 0:t.name)==="*");return n?n.args.some(t=>t==="*")?i.args:n.args.map(t=>i.args.find(s=>s.name===t)).filter(t=>!!t):[]}),d_=Os(fn.getTypeMap()),h_=s_(fn);function Os(i){return Ct.mapKeys(i||{},(n,t)=>t.toLocaleLowerCase())}function m_(){return Ct.size(fn.getTypeMap())<=10}function g_(){return[ua("Queries",fn.getQueryType()),ua("Mutations",fn.getMutationType()),ua("Subscriptions",fn.getSubscriptionType()),__(),T_()].filter(i=>!!i)}function ua(i,n){return E_(i,y_(n))}function y_(i){return Ct.sortBy((i==null?void 0:i.getFields())||{},n=>n.name)}function E_(i,n){return n.length===0?null:{type:"menu",title:i,children:n.map(t=>({type:"page",title:t.name,section:i,deprecated:!!t.deprecationReason,href:fr.joinUrlPaths(_s,i.toLocaleLowerCase(),t.name)}))}}function T_(){return m_()?null:{type:"menu",title:"Types",children:Ct.sortBy(Ct.map(fn.getTypeMap()),n=>n.name).filter(n=>!n.name.startsWith("__")).map(n=>({type:"page",title:n.name,section:"Types",href:fr.joinUrlPaths(_s,"types",n.name)}))}}function P_(){return!!fn.getQueryType()}function U_(i){return l_[i.toLocaleLowerCase()]}function $_(){return!!fn.getMutationType()}function M_(i){return c_[i.toLocaleLowerCase()]}function V_(){return!!fn.getSubscriptionType()}function G_(i){return f_[i.toLocaleLowerCase()]}function j_(i){return d_[i.toLocaleLowerCase()]}function D_(i){return xa[i.toLocaleLowerCase()]}function q_(i){return D_(i.name)!==void 0}function Y_(){return Ct.size(xa)>0}function Q_(i){return p_[i.name.toLocaleLowerCase()]||[]}function W_(i){if(i)return h_.getFor(i)}function z_(i){return Ct.flatMap(i.getFields(),n=>({field:n,possibleDescriptions:Mf(n,i)}))}function Mf(i,n){return i?i.description?[{description:i.description,from:n}]:En(n)?n.getInterfaces().flatMap(t=>Mf(t.getFields()[i.name],t)):[]:[]}function Vf(){return ya.some(i=>(i==null?void 0:i.name)==="*")?fn.getDirectives().filter(i=>!["include","skip","deprecated","specifiedBy"].includes(i.name)):ya.filter(i=>!!(i!=null&&i.name)).map(({name:i})=>i?fn.getDirective(i):void 0).filter(i=>!!i)}function v_(){return jc.trim().length===0?e_(JSON.parse(JSON.stringify({__schema:{types:[]}}))):i_(jc)}function __(){const i=Vf();return i.length===0?null:{type:"menu",title:"Directives",children:i.map(n=>({type:"page",title:n.name,href:fr.joinUrlPaths(_s,"directives",n.name),section:"Directives"}))}}const J_=wa(ba.APP_TITLE,"GraphQL Documentation"),Gf=A_().concat(g_());w_(Gf);const I_=Object.freeze(Gf),H_=b_();function A_(){return wa(ba.PAGES,F_()).filter(n=>!!n).map(n=>qf([],n))}function b_(){const i=S_();if(i)return i.href;throw new Error("No custom pages or query available to use as the root application URL. You need to provide at least one custom page or your schema should contain at least one query/mutation/subscription.")}function w_(i){function n(s,o){for(const u of s){if(u.type==="page"){o(u);continue}u.type==="menu"&&n(u.children,o)}}let t;n(i,s=>{t&&(t.next={title:s.title,section:s.section,href:s.href},s.previous={title:t.title,section:t.section,href:t.href}),t=s})}function S_(){return jf(()=>!0)}function X_(i){return jf(n=>n.href.toLocaleLowerCase()===i.toLocaleLowerCase())}function jf(i){function n(t){for(const s of t){if(s.type==="page"&&i(s))return s;if(s.type==="menu"){const o=n(s.children);if(o)return o}}return null}return n(I_)}function qf(i,n){if(typeof n.content=="string")return{type:"page",title:n.title,content:n.content,href:fr.joinUrlPaths(_s,...i,qc(n.title))};const t=i.concat([qc(n.title)]);return{type:"menu",title:n.title,children:n.content.map(s=>({...qf(t,s),section:n.title}))}}function qc(i){return fr.generatePathSegment(i,new tf)}function F_(){return[{title:"Introduction",content:[{title:"Welcome",content:`
        # Welcome 🎉
        ...
        `.split(`
`).map(i=>i.trim()).join(`
`)}]}]}export{P_ as A,tf as B,U_ as C,V_ as D,G_ as E,uv as F,ln as G,pi as H,vf as I,Df as J,j_ as K,W_ as L,z_ as M,q_ as N,I as O,B_ as P,Is as Q,ar as R,Xc as S,R_ as T,tr as U,aD as V,Ct as _,k_ as a,wa as b,Jn as c,En as d,ct as e,cn as f,Jc as g,Jt as h,Zn as i,Kn as j,et as k,m_ as l,ve as m,J_ as n,L_ as o,I_ as p,H_ as q,X_ as r,fn as s,ba as t,fr as u,Y_ as v,D_ as w,Q_ as x,$_ as y,M_ as z};
