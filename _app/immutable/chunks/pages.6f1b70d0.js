...
`,n=n.substring(S.length+1),E=!0),!E){const he=new RegExp(`^ {0,${Math.min(3,k-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),Ge=new RegExp(`^ {0,${Math.min(3,k-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),Ae=new RegExp(`^ {0,${Math.min(3,k-1)}}(?:\`\`\`|~~~)`),Ce=new RegExp(`^ {0,${Math.min(3,k-1)}}#`);for(;n;){const Re=n.split(`
`,1)[0];if(S=Re,this.options.pedantic&&(S=S.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ")),Ae.test(S)||Ce.test(S)||he.test(S)||Ge.test(n))break;if(S.search(/[^ ]/)>=k||!S.trim())d+=`
`+S.slice(k);else{if(R||I.search(/[^ ]/)>=4||Ae.test(I)||Ce.test(I)||Ge.test(I))break;d+=`
`+S}!R&&!S.trim()&&(R=!0),p+=Re+`
...
type Query {
  ...
  """
  customerTransactions(identityValue: String!, customerTransactionsInput: CustomerTransactionsInput!, pagination: PaginationQuery): CustomerTransactionsResponse @cacheControl(maxAge: 10, scope: PRIVATE)
}
...
input SubmitOrderInput {
  ...
  basket: EnrichedBasketEntityInput
...
input EnrichedBasketEntityInput {
  """Basket Analysis Date-Time in ATOM format"""
  analysedDateTime: String
  contents: [BasketContentsCollectionEntityListItemInput]
  summary: BasketSummaryInput!
  type: BasketType!
}


input BasketContentsCollectionEntityListItemInput {
  adjudicationResults: [AdjudicationResultsListItemInput]
  adjustmentResults: [AdjustmentResultsEntityListItemInput]
  adjustmentRules: AdjustmentRulesEntityInput
  bounceBackResults: [BounceBackResultsEntityListItemInput]
  """
  ...
  """
  contributionResults: [ContributionResultsEntityListItemInput]
  """Product Name"""
  description: String
  ...
  """
  qualifiesResults: [QualifiesResultsEntityListItemInput]
  """
  Recursive list of contents related to the parent item with the same schema as this object.
  """
  relatedContents: [BasketContentsCollectionEntityListItemInput]
  ...
  taxRate: Float
  tenderResults: [TenderResultsEntityListItemInput]
...
input TenderResultsEntityListItemInput {
...
input QualifiesResultsEntityListItemInput {
...
input FulfilledBaseEarnEntityListItemInput {
...
input ContributionResultsEntityListItemInput {
...
input AccountBalancesEntityInput {
  """
...
input AdjustmentRulesEntityInput {
  """
...
input AdjudicationResultsListItemInput {
  """
  The object of balances. Some balances only apply to certain types of accounts. See notes below on balances.
  """
  balances: AccountBalancesEntityInput
...
input AdjustmentResultsEntityListItemInput {
  """
...
type GiveTargetedOfferToCustomerResponse {
  result: GiveTargetedOfferToCustomer
}


type SetOfferStateResponse {
  "Details of the benefit"
  result: BenefitListItem
}
...
type VerifyAvailableRewardsResponse {
  ...
  result: PointsAndRewardsAvailable
}
...
type GiveTargetedOfferToCustomer {
...
`;class rv{constructor(n){De(this,"mapping");this.mapping=n}getFor(n){return this.mapping.get(n.name)}}var ar;(function(i){i.UNION="union",i.ARGUMENT="argument",i.FIELD="field"})(ar||(ar={}));function sv(i){const n=new Map;return ov(i,t=>{En(t)&&av(t,s=>{const o=ga(s.type);aa(n,o).references.push({kind:ar.FIELD,parent:t,by:s}),uv(s,l=>{const p=ga(l.type);aa(n,p).references.push({kind:ar.ARGUMENT,field:s,type:t,by:l})})}),Kn(t)&&t.getTypes().forEach(s=>{aa(n,s).references.push({kind:ar.UNION,by:t})})}),new rv(n)}function ov(i,n){Object.entries(i.getTypeMap()).forEach(([,t])=>{t.name.startsWith("__")||n(t)})}function av(i,n){Object.entries(i.getFields()).forEach(([,t])=>{n(t)})}function uv(i,n){Object.entries(i.args).forEach(([,t])=>{n(t)})}function aa(i,n){let t=i.get(n.name);return t||(t={references:[]},i.set(n.name,t)),t}function ga(i){return ln(i)||pi(i)?ga(i.ofType):i}const fn=Iv(),ya=wa(Aa.DIRECTIVES,[]);var Yc;const lv=Os((Yc=fn.getQueryType())==null?void 0:Yc.getFields());var Qc;const cv=Os((Qc=fn.getMutationType())==null?void 0:Qc.getFields());var Wc;const fv=Os((Wc=fn.getSubscriptionType())==null?void 0:Wc.getFields()),ka=Ct.keyBy(Vf(),i=>i.name.toLocaleLowerCase()),pv=Ct.mapValues(ka,i=>{const n=ya.find(t=>(t==null?void 0:t.name)===i.name||(t==null?void 0:t.name)==="*");return n?n.args.some(t=>t==="*")?i.args:n.args.map(t=>i.args.find(s=>s.name===t)).filter(t=>!!t):[]}),dv=Os(fn.getTypeMap()),hv=sv(fn);function Os(i){return Ct.mapKeys(i||{},(n,t)=>t.toLocaleLowerCase())}function mv(){return Ct.size(fn.getTypeMap())<=10}function gv(){return[ua("Queries",fn.getQueryType()),ua("Mutations",fn.getMutationType()),ua("Subscriptions",fn.getSubscriptionType()),vv(),Tv()].filter(i=>!!i)}function ua(i,n){return Ev(i,yv(n))}function yv(i){return Ct.sortBy((i==null?void 0:i.getFields())||{},n=>n.name)}function Ev(i,n){return n.length===0?null:{type:"menu",title:i,children:n.map(t=>({type:"page",title:t.name,section:i,deprecated:!!t.deprecationReason,href:fr.joinUrlPaths(vs,i.toLocaleLowerCase(),t.name)}))}}function Tv(){return mv()?null:{type:"menu",title:"Types",children:Ct.sortBy(Ct.map(fn.getTypeMap()),n=>n.name).filter(n=>!n.name.startsWith("__")).map(n=>({type:"page",title:n.name,section:"Types",href:fr.joinUrlPaths(vs,"types",n.name)}))}}function Pv(){return!!fn.getQueryType()}function Uv(i){return lv[i.toLocaleLowerCase()]}function $v(){return!!fn.getMutationType()}function Mv(i){return cv[i.toLocaleLowerCase()]}function Vv(){return!!fn.getSubscriptionType()}function Gv(i){return fv[i.toLocaleLowerCase()]}function jv(i){return dv[i.toLocaleLowerCase()]}function Dv(i){return ka[i.toLocaleLowerCase()]}function qv(i){return Dv(i.name)!==void 0}function Yv(){return Ct.size(ka)>0}function Qv(i){return pv[i.name.toLocaleLowerCase()]||[]}function Wv(i){if(i)return hv.getFor(i)}function zv(i){return Ct.flatMap(i.getFields(),n=>({field:n,possibleDescriptions:Mf(n,i)}))}function Mf(i,n){return i?i.description?[{description:i.description,from:n}]:En(n)?n.getInterfaces().flatMap(t=>Mf(t.getFields()[i.name],t)):[]:[]}function Vf(){return ya.some(i=>(i==null?void 0:i.name)==="*")?fn.getDirectives().filter(i=>!["include","skip","deprecated","specifiedBy"].includes(i.name)):ya.filter(i=>!!(i!=null&&i.name)).map(({name:i})=>i?fn.getDirective(i):void 0).filter(i=>!!i)}function Iv(){return jc.trim().length===0?ev(JSON.parse(JSON.stringify({__schema:{types:[]}}))):iv(jc)}function vv(){const i=Vf();return i.length===0?null:{type:"menu",title:"Directives",children:i.map(n=>({type:"page",title:n.name,href:fr.joinUrlPaths(vs,"directives",n.name),section:"Directives"}))}}const Jv=wa(Aa.APP_TITLE,"GraphQL Documentation"),Gf=bv().concat(gv());wv(Gf);const _v=Object.freeze(Gf),Hv=Av();function bv(){return wa(Aa.PAGES,Fv()).filter(n=>!!n).map(n=>qf([],n))}function Av(){const i=Sv();if(i)return i.href;throw new Error("No custom pages or query available to use as the root application URL. You need to provide at least one custom page or your schema should contain at least one query/mutation/subscription.")}function wv(i){function n(s,o){for(const u of s){if(u.type==="page"){o(u);continue}u.type==="menu"&&n(u.children,o)}}let t;n(i,s=>{t&&(t.next={title:s.title,section:s.section,href:s.href},s.previous={title:t.title,section:t.section,href:t.href}),t=s})}function Sv(){return jf(()=>!0)}function Xv(i){return jf(n=>n.href.toLocaleLowerCase()===i.toLocaleLowerCase())}function jf(i){function n(t){for(const s of t){if(s.type==="page"&&i(s))return s;if(s.type==="menu"){const o=n(s.children);if(o)return o}}return null}return n(_v)}function qf(i,n){if(typeof n.content=="string")return{type:"page",title:n.title,content:n.content,href:fr.joinUrlPaths(vs,...i,qc(n.title))};const t=i.concat([qc(n.title)]);return{type:"menu",title:n.title,children:n.content.map(s=>({...qf(t,s),section:n.title}))}}function qc(i){return fr.generatePathSegment(i,new tf)}function Fv(){return[{title:"Introduction",content:[{title:"Welcome",content:`
        # Welcome 🎉
        ...
        `.split(`
`).map(i=>i.trim()).join(`
`)}]}]}export{Pv as A,tf as B,Uv as C,Vv as D,Gv as E,uI as F,ln as G,pi as H,If as I,Df as J,jv as K,Wv as L,zv as M,qv as N,_ as O,Bv as P,_s as Q,ar as R,Xc as S,Rv as T,tr as U,aD as V,Ct as _,xv as a,wa as b,Jn as c,En as d,ct as e,cn as f,Jc as g,Jt as h,Zn as i,Kn as j,et as k,mv as l,Ie as m,Jv as n,Lv as o,_v as p,Hv as q,Xv as r,fn as s,Aa as t,fr as u,Yv as v,Dv as w,Qv as x,$v as y,Mv as z};
